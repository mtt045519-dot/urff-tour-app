...
export default function App() {
  ...
  // Keep the opening/refresh splash visible for a full 5 seconds.
  // Because this component mounts again on browser refresh (F5/Ctrl+R), the same
  // 5-second animation automatically plays after every refresh as well.
  useEffect(() => {
  ...
  // transaction so simultaneous user/admin sends cannot overwrite each other.
  const setSupportChats = (updater) => {
    _setSupportChats(prev => (typeof updater === 'function' ? updater(prev) : updater));
  };

  const appendSupportMessage = async (uid, message) => {
    if (!uid || !message) return;
    _setSupportChats(prev => {
      const thread = prev[uid] || { messages: [], aiEnabled: true };
      return { ...prev, [uid]: { ...thread, messages: [...(thread.messages || []), message] } };
    });
    try {
      await runTransaction(db, async (transaction) => {
        const ref = doc(db, 'supportChats', uid);
        const snap = await transaction.get(ref);
        const current = snap.exists() ? snap.data() : { messages: [], aiEnabled: true };
        const messages = Array.isArray(current.messages) ? current.messages : [];
        transaction.set(ref, { ...current, messages: [...messages, message], updatedAtMs: Date.now() }, { merge: true });
      });
    } catch (e) { console.error('Support message save error:', e); }
  };
  ...
  const [tournaments, setTournaments] = useState([
   ...
   useEffect(() => {
    ...
    }, (err) => console.error('Tournament sync error:', err));
    return () => unsub();
  }, []);
 
  // Daily automation safety-net. The primary next-day clone is created when the
  // admin marks a match started; this listener also repairs missed days after a
  // refresh or when the browser was closed. New matches never copy joinEntries.
  useEffect(() => {
    ...
    const generateDueMatches = async () => {
      ...
      for (const tpl of automatedTemplates) {
        if (tpl.lastGeneratedDate && tpl.lastGeneratedDate >= dateKey) continue;
        const matchId = `m_auto_${tpl.id}_${dateKey}`;
        const newMatch = { id: matchId, templateId: tpl.id, generatedDate: dateKey, category: tpl.category, categoryId: tpl.categoryId, title: tpl.title, time: `${dateKey} ${tpl.timeLabel}`, prizePool: tpl.prizePool, perKill: tpl.perKill, entryFee: tpl.entryFee, map: tpl.map || 'BERMUDA', mode: tpl.mode, status: 'upcoming', started: false, slotsFilled: 0, totalSlots: tpl.totalSlots, image: tpl.image, rules: tpl.rules, roomInfo: { id: '', pass: '' }, prizeTable: tpl.prizeTable || [] };
        try {
          const ref = doc(db, 'tournaments', matchId);
          await runTransaction(db, async (transaction) => {
            const snap = await transaction.get(ref);
            if (!snap.exists()) transaction.set(ref, newMatch);
          });
          await setDoc(doc(db, 'automatedTemplates', tpl.id), { ...tpl, lastGeneratedDate: dateKey }, { merge: true });
          setTournaments(prev => prev.some(m => m.id === matchId) ? prev : [...prev, newMatch]);
        } catch (e) { console.error('Daily match clone error:', e); }
      }
    };
    generateDueMatches();
  ...
  const handleBannerImageUpload = async (e) => {
    ...
    try {
      if (file.type.startsWith('video/')) {
        ...
        showToast('Shudhu image ba video upload kora jabe');
      }
    } catch (err) {
  ...
  const loadBannerFormFromSlot = (slotIdx) => {
    const b = banners[slotIdx] || { image: '', title: '', subtitle: '', mediaType: 'image' };
  ...
  const handleSaveTournament = () => {
    ...
    const cleanPrizeTable = newTourPrizeTable
      ...
      setTournaments(tournaments.map(m => m.id === editingTourId ? fullyUpdated : m));
      setDoc(doc(db, 'tournaments', editingTourId), fullyUpdated).catch(e => console.error(e));
  ...
  const cloneMatchForNextDay = async (sourceMatch) => {
    const template = automatedTemplates.find(tpl => tpl.id === sourceMatch.templateId) || automatedTemplates.find(tpl => tpl.sourceTitle === sourceMatch.title);
    if (!template) return null;
    const sourceDate = parseMatchTime(sourceMatch.time) || new Date();
    const nextDate = new Date(sourceDate.getTime());
    nextDate.setDate(nextDate.getDate() + 1);
    const nextDateKey = dateKeyFromDate(nextDate);
    const matchId = `m_auto_${template.id}_${nextDateKey}`;
    const timeLabel = template.timeLabel || ((sourceMatch.time || '').trim().split(' ').slice(-2).join(' '));
    const newMatch = { id: matchId, templateId: template.id, generatedDate: nextDateKey, category: template.category, categoryId: template.categoryId, title: template.title, time: `${nextDateKey} ${timeLabel}`, prizePool: template.prizePool, perKill: template.perKill, entryFee: template.entryFee, map: template.map || 'BERMUDA', mode: template.mode, status: 'upcoming', started: false, slotsFilled: 0, totalSlots: template.totalSlots, image: template.image, rules: template.rules, roomInfo: { id: '', pass: '' }, prizeTable: template.prizeTable || [] };
    ...
    await setDoc(doc(db, 'automatedTemplates', template.id), { ...template, lastGeneratedDate: nextDateKey }, { merge: true });
    setTournaments(prev => prev.some(m => m.id === matchId) ? prev : [...prev, newMatch]);
    return newMatch;
  };

  const handleToggleMatchStarted = async (id) => {
    const match = tournaments.find(m => m.id === id);
    if (!match) return;
    const nextStarted = !match.started;
    const changedT = { ...match, started: nextStarted, status: nextStarted ? 'live' : match.status };
    try {
      await setDoc(doc(db, 'tournaments', id), changedT, { merge: true });
      setTournaments(prev => prev.map(m => m.id === id ? changedT : m));
      if (nextStarted) {
        const next = await cloneMatchForNextDay(changedT);
        showToast(next ? 'Match started + next-day fresh match scheduled (0 participants).' : 'Match started. Daily automation template not found.');
      } else {
...
${buildUserContextBrief(uid)}`;
  ...
  const fetchAiSupportReply = async (uid, conversation) => {
    try {
      ...
      await appendSupportMessage(uid, { sender: 'ai', text: replyText, time: 'Just now', createdAtMs: Date.now() });
    } catch (err) {
      await appendSupportMessage(uid, { sender: 'ai', text: 'Ekhon assistant e shomossha hocche. Admin ke sরাসরি contact korun ba pore try korun.', time: 'Just now', createdAtMs: Date.now() });
    }
  };
  ...
  const handleSendSupportMessage = () => {
    ...
    appendSupportMessage(uid, message);
    setSupportChatInput('');
  ...
  const appendUserChatMessage = (uid, message, triggerAi) => {
    const existing = supportChats[uid] || { messages: [], aiEnabled: true };
    const normalizedMessage = { ...message, createdAtMs: message.createdAtMs || Date.now() };
    const updatedMessages = [...(existing.messages || []), normalizedMessage];
    appendSupportMessage(uid, normalizedMessage);
  ...
  const handleAdminChatImageUpload = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const message = { sender: 'admin', type: 'image', imageUrl: ev.target.result, text: '', time: 'Just now', createdAtMs: Date.now() };
      appendSupportMessage(uid, message);
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        reader.onloadend = () => {
          ...
          const message = { sender: forAdmin ? 'admin' : 'user', type: 'voice', audioUrl, audioMimeType: mimeType, transcript, text: transcript, time: 'Just now', createdAtMs: Date.now() };
  ...
  const toggleAiForThread = async (uid) => {
    const thread = supportChats[uid] || { messages: [], aiEnabled: true };
    const next = { ...thread, aiEnabled: !(thread.aiEnabled !== false), updatedAtMs: Date.now() };
    _setSupportChats(prev => ({ ...prev, [uid]: next }));
    try {
      await setDoc(doc(db, 'supportChats', uid), next, { merge: true });
    } catch (e) { console.error('Support AI toggle save error:', e); }
  };
          ...
          {adminTab === 'banner' && (
            <div className="space-y-4">
              ...
              <div className={`${t.card} border ${t.border} rounded-2xl p-4 space-y-3`}>
                ...
                >
                  <Upload className="w-4 h-4 text-indigo-400" />
                  <span>Gallery theke Banner Image / Video Dao</span>
                </button>
                <input
                        ...
                        <div key={idx} className={`flex ${m.sender === 'user' ? 'justify-start' : 'justify-end'}`}>
                          <div className={`max-w-[80%] rounded-xl px-3 py-2 text-xs ${
                            ...
                            {m.type === 'voice' && m.audioUrl && (
                              <audio controls preload="metadata" src={m.audioUrl} className="max-w-full mb-1" style={{ height: 38 }} onError={(e) => e.currentTarget.setAttribute('data-audio-error', 'true')} />
                            )}
